...
WebSocketWorker::~WebSocketWorker() { stopServer(); }

void WebSocketWorker::setDatabasePath(const QString &dbPath) { m_dbPath = dbPath; }
void WebSocketWorker::setSecret(const QString &secret) { m_secret = secret; }
bool WebSocketWorker::isRunning() const { return m_running; }
int WebSocketWorker::clientCount() const { return m_clients.size(); }
...
bool WebSocketWorker::startServer(quint16 port, bool useWss, const QString &certPath, const QString &keyPath) {
    ...
    if (useWss) {
        m_server = new QWebSocketServer("SIEM Agent WSS", QWebSocketServer::SecureMode, this);
        ...
        m_server = new QWebSocketServer("SIEM Agent WS", QWebSocketServer::NonSecureMode, this);
    }
    ...
    if (m_server->listen(QHostAddress::Any, port)) {
        ...
        qDebug() << "[WSS] Сервер запущен на порту" << port << (useWss ? "(WSS)" : "(WS)");
        return true;
    }
...
void WebSocketWorker::onNewConnection() {
    ...
    connect(socket, &QWebSocket::disconnected, this, &WebSocketWorker::onClientDisconnected);
    m_clients << socket;
    emit clientCountChanged();
}
