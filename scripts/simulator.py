...
WS_HOST       = os.environ.get("SIEM_HOST", "localhost")
WS_PORT       = int(os.environ.get("SIEM_PORT", "8080"))
SEND_INTERVAL = float(os.environ.get("EVENTS_PER_SECOND", "2.0"))
...
async def run_simulator():
    ...
    try:
        async with websockets.connect(uri, ssl=ssl_ctx) as ws:
            print(f"Подключено.  Отправка событий каждые {SEND_INTERVAL}с\n")
            while True:
                event = generate_event()
                event = sign_event(event, SHARED_SECRET)
                await ws.send(json.dumps(event, ensure_ascii=False))
                sent += 1
                print(f"[{sent:04d}] [{event['severity'].upper():8s}] "
                      f"{event['deviceName']} — {event['action']}")
        ...
        print(f"\nОстановлено вручную. Отправлено событий: {sent}")
